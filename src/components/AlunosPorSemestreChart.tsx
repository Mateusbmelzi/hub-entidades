...
import { PieChart, TrendingUp, TrendingDown, RefreshCw, Clock } from 'lucide-react';

interface AlunoPorSemestre {
  semestre: number;
  total_alunos: number;
}

interface AlunosPorSemestreChartProps {
  alunosPorSemestre: AlunoPorSemestre[];
  loading: boolean;
  error: string | null;
  onRefetch: () => void;
}

export const AlunosPorSemestreChart: React.FC<AlunosPorSemestreChartProps> = ({
  alunosPorSemestre,
  loading,
  error,
  onRefetch
  ...
  const totalAlunos = alunosPorSemestre.reduce((sum, item) => sum + item.total_alunos, 0);
  const calculateAngles = () => {
    let currentAngle = 0;
    return alunosPorSemestre.map((item, index) => {
      const percentage = item.total_alunos / totalAlunos;
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Distribuição dos estudantes por semestre
          </p>
        </CardHeader>
        <CardContent>
              ...
              <svg width="200" height="200" viewBox="-100 -100 200 200" className="transform -rotate-90">
                {pieData.map((slice, index) => (
                  <path
                    ...
                    title={`${slice.semestre}º Semestre: ${slice.total_alunos} alunos (${(slice.percentage * 100).toFixed(1)}%)`}
                  />
                ))}
              </svg>
                ...
                <div key={index} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-gray-900 truncate">
                      {slice.semestre}º Semestre
                    </div>
                    <div className="text-xs text-gray-500">
                      {slice.total_alunos} aluno{slice.total_alunos !== 1 ? 's' : ''}
                    </div>
                  </div>
