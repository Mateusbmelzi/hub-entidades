...
import { PieChart, TrendingUp, TrendingDown, RefreshCw, GraduationCap } from 'lucide-react';

interface AlunoPorCurso {
  curso: string;
  total_alunos: number;
}

interface AlunosPorCursoChartProps {
  alunosPorCurso: AlunoPorCurso[];
  loading: boolean;
  error: string | null;
  onRefetch: () => void;
}

export const AlunosPorCursoChart: React.FC<AlunosPorCursoChartProps> = ({
  alunosPorCurso,
  loading,
  error,
  onRefetch
  ...
  const totalAlunos = alunosPorCurso.reduce((sum, item) => sum + item.total_alunos, 0);
  const calculateAngles = () => {
    let currentAngle = 0;
    return alunosPorCurso.map((item, index) => {
              ...
              <svg width="200" height="200" viewBox="-100 -100 200 200" className="transform -rotate-90">
                {pieData.map((slice, index) => (
                  <path
                    ...
                    title={`${slice.curso}: ${slice.total_alunos} alunos (${(slice.percentage * 100).toFixed(1)}%)`}
                  />
                ))}
              </svg>
